...
<div className="flex flex-nowrap justify-between items-center mt-[1vh] gap-[1vw] w-full overflow-x-auto">
  ...
  <div className="flex-1 flex justify-center">
      <div className="bg-blue-100 text-blue-900 rounded px-[1vw] py-[0.6vh] text-[clamp(0.7rem,1vw,1.1rem)] font-medium shadow-sm text-center whitespace-nowrap">
        Sıra şimdi <span className="font-bold">{siradakiKisi()}</span>'de/da, size sıra gelmesine <span className="font-bold">{kalanKisiSayisi()}</span> kişi var.
      </div>
    </div>
</div>
    ...
    {activeList.map((emp, i) => (
      <div
        ...
        className={clsx(
          ...
          i === siradakiMusaitIndex() && "border-4 border-green-600 bg-gray-50"
        )}
        style={{maxHeight: '8vh'}}
      >
        <div className="flex flex-wrap items-center justify-between gap-[0.5vw] min-h-[3vh]">
          ...
          {emp.uid === auth.currentUser?.uid ? (
            <div className="flex flex-wrap gap-[0.3vw]">
              ...
              <button onClick={() => durumGuncelle(i, "Çalışıyor")} className="px-[0.7vw] py-[0.3vh] bg-orange-400 text-black rounded text-[clamp(0.6rem,0.6vw,0.8rem)]">Çalışıyorum</button>
              <button onClick={() => durumGuncelle(i, "Müsait")} className="px-[0.7vw] py-[0.3vh] bg-green-400 text-black rounded text-[clamp(0.6rem,0.6vw,0.8rem)]">Müsaitim</button>
            </div>
          ) : (
